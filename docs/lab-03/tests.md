...
| MIG-01 | Migration/Regression | FR-24, BR-36, BR-37, BR-38, BR-39; AC-07 | Lab 2 Requester -> User migration/provisioning | Exact legacy ids/FKs/data/counts remain valid; email normalization collision preflight is enforced; Ticket backfills/nullability/default/FK/onDelete/index invariants match Section 7.1/7.2; existing globally unique `Ticket.clientRequestId` values/constraint are preserved; first credential provisioning is hashed; rerun does not reset changed credentials | `server/tests/lab-03/migration.integration.test.ts` | Pending |
