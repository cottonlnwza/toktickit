...
| API-18 | API | FR-13, BR-18; AC-13 | Claim owner | Unassigned Ticket claimed by current active IT Staff; already-owned claim conflicts safely; concurrent claimant deactivation/demotion is serialized and cannot persist an ineligible owner | `server/tests/lab-03/staff-ticket-detail.api.test.ts` | Pass — Issue #38 local isolated PostgreSQL / Node 22 |
| API-19 | API | FR-13, BR-18; AC-13 | Assign/reassign/unassign | Only active IT Staff/Admin target accepted; inactive/Requester/missing targets rejected; concurrent target deactivation/demotion is serialized before persistence | `server/tests/lab-03/staff-ticket-detail.api.test.ts` | Pass — Issue #38 local isolated PostgreSQL / Node 22 |
| API-20 | API/Security | FR-14, BR-20, BR-21; AC-14 | IT Priority | IT Staff/Admin update allowed; Requested Priority unchanged; Requester forbidden | `server/tests/lab-03/staff-ticket-detail.api.test.ts` | Pass — Issue #38 local isolated PostgreSQL / Node 22 |
| API-21 | API/Security | FR-15, BR-23, BR-24; AC-15 | Status transitions | Valid IT Staff matrix transition passes under row lock; invalid/Requester/Admin transition rejected | `server/tests/lab-03/staff-ticket-detail.api.test.ts` | Pass — Issue #38 local isolated PostgreSQL / Node 22 |
| API-22 | API | FR-16, BR-29; AC-16 | Comment validation/render contract | Blank/over-limit rejected; markup stored as inert plain text; Staff author/time backend-controlled | `server/tests/lab-03/comments-notes.api.test.ts` | Pass — Issue #38 local isolated PostgreSQL / Node 22 |
...
| API-24 | API/Security | FR-17; AC-18 | Internal Note permitted roles | IT Staff creates/reads; Admin reads only; Requester canonical access forbidden; backend author/time + validation | `server/tests/lab-03/comments-notes.api.test.ts` | Pass — Issue #38 local isolated PostgreSQL / Node 22 |
| API-25 | API/Regression | FR-12; AC-19 | Staff Detail Attachment continuity | Existing Attachment safe metadata/download remain available without storage-path leakage | `server/tests/lab-03/staff-ticket-detail.api.test.ts` | Pass — Issue #38 local isolated PostgreSQL / Node 22 |
| API-26 | API | FR-18, FR-19; AC-20 | Admin list/search/role filter | Safe user rows; case-insensitive name/email search; role filter; unknown/repeated/invalid query rejected | `server/tests/lab-03/users-admin.api.test.ts` | Pass — Issue #39 local isolated PostgreSQL / Node 22 |
| API-27 | API | FR-20; AC-21 | Admin create User | One valid role; normalized unique email; salted scrypt initial password; change flag true | `server/tests/lab-03/users-admin.api.test.ts` | Pass — Issue #39 local isolated PostgreSQL / Node 22 |
| API-28 | API/Security | FR-23; AC-22 | Duplicate email/invalid role/input | 400/409 safe rejection; no partial User; no delete endpoint; password fields rejected from profile PATCH | `server/tests/lab-03/users-admin.api.test.ts` | Pass — Issue #39 local isolated PostgreSQL / Node 22 |
| API-29 | API/Security | FR-21, FR-23; AC-22 | Self-deactivation / last active Admin | Self-deactivation conflict and row-locked last-active-Admin floor, including concurrent demotion regression | `server/tests/lab-03/users-admin.api.test.ts` | Pass — Issue #39 local isolated PostgreSQL / Node 22 |
| API-30 | API/Security | FR-21, FR-22; AC-23 | Edit User/new initial password | Allowed profile fields only; deactivation/role/password reset revokes sessions; ineligible owner is unassigned; reset hash + change flag; self/current-password reset rejected | `server/tests/lab-03/users-admin.api.test.ts` | Pass — Issue #39 local isolated PostgreSQL / Node 22 |
| API-31 | API/Security | FR-23; AC-24 | Non-Admin User Management | Requester/IT Staff receive 403 with no protected user-list/account data | `server/tests/lab-03/users-admin.api.test.ts` | Pass — Issue #39 local isolated PostgreSQL / Node 22 |
...
Issue #38 IT Staff Ticket Operations followed TDD. The planned status/comment-note unit tests, Staff Detail API tests, canonical Internal Note API tests, and `StaffTicketDetail.test.tsx` were added before production implementation. The legitimate red state showed the new Staff Detail/claim/owner/priority/status/Internal Note routes returning `404`, while direct `#staff-ticket-<id>` navigation still rendered the existing Queue or Administrator placeholder. Implementation then stayed inside Issue #38 scope; Administrator User Management and Actions Taken remain deferred.

- Focused server: `ticket-status.unit.test.ts`, `comments-notes.unit.test.ts`, `staff-ticket-detail.api.test.ts`, `comments-notes.api.test.ts` = 4 files / 51 tests passed after the post-merge ownership-race correction.
- Focused client: `StaffTicketDetail.test.tsx` = 1 file / 6 tests passed after PR #50 review correction.
- Full server regression suite: 22 files / 164 tests passed using the isolated Lab 3 suite/migration databases after the post-merge ownership-race correction.
- Full client regression suite: 16 files / 85 tests passed after PR #50 review correction.
- `npm run build --prefix server`: passed.
- `npm run build --prefix client`: passed.
- `prisma validate`: passed.
- `git diff --check`: passed.
- All counts above are local verification only; no hosted CI result is claimed.

Issue #38 verification covers Staff/Administrator direct-detail role boundaries; safe grouped Ticket/Requester/classification payloads; stable active IT Staff/Administrator owner choices; atomic claim conflict handling; assign/reassign/unassign validation; Requested Priority immutability; IT Priority authority for IT Staff/Administrator; the exact BR-24 status matrix under a row lock; required UI confirmations; backend-authored Public Comments/Internal Notes with 1-2000 trimmed plain-text validation; Requester denial from Internal Notes/Staff Detail operations; visually distinct Public/Internal UI; Lab 2 Attachment safe metadata/download continuity without storage-path leakage; Requester `Problem Appears Resolved` visibility without formal status mutation; Administrator oversight with only IT Priority editable; and safe loading/validation/forbidden/conflict/not-found/unexpected-failure feedback. PR #50 review `5255008801` exposed the domain-feedback gap and was corrected on `433f0e4`. A second exact-HEAD review `5255066969` then identified an ownership eligibility race: middleware authorization could become stale before claim persistence. The follow-up regression first reproduced that merged bug, then proved claim waits on concurrent claimant deactivation/demotion, re-checks active `IT_STAFF` eligibility in the same row-locked transaction as Ticket assignment, and leaves `ownerId=NULL` when eligibility is lost. The same BR-18 serialization is applied and regression-tested for assign/reassign target eligibility.

### Issue #39 execution evidence

Issue #39 Administrator User Management followed TDD. `users-admin.api.test.ts` and `UserManagement.test.tsx` were added first. The legitimate red state was 9/9 server failures because every `/api/admin/users...` route returned `404`, plus 4/4 client failures because the Administrator shell still rendered only the authenticated-session placeholder and non-Administrator direct `#user-management` access had no forbidden screen. Production implementation then stayed inside the minimalist User Management contract.

- Focused server: `users-admin.api.test.ts` = 1 file / 14 tests passed.
- Focused client: `UserManagement.test.tsx` = 1 file / 8 tests passed.
- Focused shell + User Management regression: 2 files / 14 tests passed.
- Full server regression suite: 23 files / 178 tests passed using isolated Lab 3 suite/migration databases.
- Full client regression suite: 17 files / 93 tests passed after updating the existing shell assertion to identify the role badge specifically now that the User Management role-filter option also contains `Administrator` text.
- `npm run build --prefix server`: passed.
- `npm run build --prefix client`: passed.
- `prisma validate`: passed.
- `git diff --check`: passed.
- Normal development DB safety check remained at the Lab 2 baseline: no Lab 3 `User` table; 5 `RequesterUser`, 99 `Ticket`, 91 `Attachment` rows.
- All counts above are local verification only; no hosted CI result is claimed.
