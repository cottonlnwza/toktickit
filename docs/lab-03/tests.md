...
| MIG-01 | Migration/Regression | FR-24, BR-36, BR-37, BR-39; AC-07 | Lab 2 Requester -> User migration/provisioning | Exact legacy ids/FKs/data remain valid; first credential provisioning is hashed; rerun does not reset changed credentials | `server/tests/lab-03/migration.integration.test.ts` | Pending |
...
| API-32 | API/Regression | FR-08, BR-38; AC-08 | Ticket create idempotency | First UUID create is 201; identical replay returns original Ticket with 200; conflicting reuse returns 409; only one Ticket row exists | `server/tests/lab-03/requester-regression.api.test.ts` | Pending |
