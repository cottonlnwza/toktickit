...
| API-18 | API | FR-13, BR-18; AC-13 | Claim owner | Unassigned Ticket claimed by current active IT Staff; already-owned claim conflicts safely | `server/tests/lab-03/staff-ticket-detail.api.test.ts` | Pass — Issue #38 local isolated PostgreSQL / Node 22 |
| API-19 | API | FR-13, BR-18; AC-13 | Assign/reassign/unassign | Only active IT Staff/Admin target accepted; inactive/Requester/missing targets rejected | `server/tests/lab-03/staff-ticket-detail.api.test.ts` | Pass — Issue #38 local isolated PostgreSQL / Node 22 |
...
- Focused server: `ticket-status.unit.test.ts`, `comments-notes.unit.test.ts`, `staff-ticket-detail.api.test.ts`, `comments-notes.api.test.ts` = 4 files / 47 tests passed.
- Focused client: `StaffTicketDetail.test.tsx` = 1 file / 6 tests passed after PR #50 review correction.
- Full server regression suite: 22 files / 160 tests passed using the isolated Lab 3 suite/migration databases.
