...
| #35 Authentication | `feature/lab3-3-authentication` | [PR #46](https://github.com/cottonlnwza/toktickit/pull/46) | Chartanat Upthaipiboon (`@Chxtamos`, `67070507210`); Tanboon Teawsawat (`@Tanaboonnnnn`, `67070507211`) | Reviews [`5247237227`](https://github.com/cottonlnwza/toktickit/pull/46#pullrequestreview-5247237227), [`5247342153`](https://github.com/cottonlnwza/toktickit/pull/46#pullrequestreview-5247342153), and [`5247449235`](https://github.com/cottonlnwza/toktickit/pull/46#pullrequestreview-5247449235) requested production password-gate evidence, concurrency-safe Change Password, and frozen BR-10 throttle-window semantics respectively. Review [`5248249098`](https://github.com/cottonlnwza/toktickit/pull/46#pullrequestreview-5248249098) on exact HEAD `671336d` found the Authentication Foundation itself sound but blocked approval because the normal production Requester shell had replaced Lab 2 Create Ticket/My Tickets/Ticket Detail/Attachment workflows with a placeholder while those flows existed only behind the legacy test harness. | Local correction restores production continuity by rendering the shared Requester workflow inside the authenticated Requester shell with the current authenticated User fixed as the Requester identity. The Development Requester selector/Change Requester controls and `/api/requesters` call remain absent from the production path; Create Ticket, My Tickets, Ticket Detail, and Attachment UI are verified through the default `App`. Backend ownership rewiring remains Issue #36 scope. Correction is not yet committed/pushed or re-reviewed. | Pending | Pending |
