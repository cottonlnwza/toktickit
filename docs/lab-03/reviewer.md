...
| #36 Authorization and Requester Regression | `feature/lab3-4-authorization-requester-regression` | [PR #47](https://github.com/cottonlnwza/toktickit/pull/47) | Tanboon Teawsawat (`@Tanaboonnnnn`, `67070507211`) | Review [`5250377027`](https://github.com/cottonlnwza/toktickit/pull/47#pullrequestreview-5250377027) on exact HEAD `9bca7ad` confirmed the ownership/authorization/idempotency work but requested one lifecycle blocker: `Problem Appears Resolved` had no backend state eligibility rule, so it could be called on `NEW`, terminal, or otherwise inappropriate states. | Correction freezes BR-42: only `OPEN`, `IN_PROGRESS`, `WAITING_FOR_REQUESTER`, and `REOPENED` are eligible because those are the BR-24 states from which IT Staff may formally transition to `RESOLVED`; `NEW`, `RESOLVED`, `CLOSED`, and `CANCELLED` return `409 RESOLUTION_INDICATION_NOT_ALLOWED` without mutation. Positive/negative backend tests and an ineligible-state UI test were added. Peer re-review remains pending. | Pending | Pending |
