...
| #35 Authentication | `feature/lab3-3-authentication` | [PR #46](https://github.com/cottonlnwza/toktickit/pull/46) | Chartanat Upthaipiboon (`@Chxtamos`, `67070507210`) | Review [`5247237227`](https://github.com/cottonlnwza/toktickit/pull/46#pullrequestreview-5247237227) on exact HEAD `7c0960d` requested one blocker correction: prove the mandatory first-login password gate at a real exported production application route instead of only a throwaway Express test route. | Follow-up correction wires `requireNormalAccess` to production `GET /api/categories`; API-06 now proves 403 `PASSWORD_CHANGE_REQUIRED` before password change and successful access after password change through the exported production `app`. The existing Lab 1 category regression test now authenticates a normal-access test user. Peer re-review remains pending. | Pending | Pending |
