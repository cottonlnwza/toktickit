...
| #35 Authentication | `feature/lab3-3-authentication` | [PR #46](https://github.com/cottonlnwza/toktickit/pull/46) | Chartanat Upthaipiboon (`@Chxtamos`, `67070507210`) | Review [`5247237227`](https://github.com/cottonlnwza/toktickit/pull/46#pullrequestreview-5247237227) on `7c0960d` requested production-route evidence for the first-login gate. Re-review [`5247342153`](https://github.com/cottonlnwza/toktickit/pull/46#pullrequestreview-5247342153) on `249c9ff` confirmed that blocker closed, then identified a concurrent Change Password race where two requests could both verify the old hash and one successful response could receive a session immediately revoked by the other request. | The production gate correction remains in place. The next correction uses an optimistic conditional User update tied to the exact verified `passwordHash`; only one concurrent change can win before session revocation/fresh-session creation. API-07 concurrently submits two valid-session password changes and proves one winner, winner password/hash/session validity, old-session revocation, and no loser overwrite. Peer re-review remains pending. | Pending | Pending |
