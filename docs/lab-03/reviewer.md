...
| #36 Authorization and Requester Regression | `feature/lab3-4-authorization-requester-regression` | [PR #47](https://github.com/cottonlnwza/toktickit/pull/47) | Tanboon Teawsawat (`@Tanaboonnnnn`, `67070507211`) | Review [`5250377027`](https://github.com/cottonlnwza/toktickit/pull/47#pullrequestreview-5250377027) on `9bca7ad` requested lifecycle eligibility for `Problem Appears Resolved`. Review [`5250628688`](https://github.com/cottonlnwza/toktickit/pull/47#pullrequestreview-5250628688) on exact HEAD `239dff2` confirmed that rule but found a race because eligibility was read before a separate write, allowing a concurrent Staff transition to an ineligible status between the two operations. | Follow-up correction makes ownership lookup, status eligibility, idempotency check, and first indication write one row-locked transaction. A concurrency regression holds a Staff transition lock, starts the Requester indication request, commits `OPEN -> RESOLVED` first, and verifies the Requester receives `409 RESOLUTION_INDICATION_NOT_ALLOWED` with no indication fields written. Peer re-review remains pending. | Pending | Pending |
