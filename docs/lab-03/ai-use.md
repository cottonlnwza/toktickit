...
10. Implement Issue #37 IT Staff Ticket Queue using TDD and apply PR #48 review `5251567480`: keep the IT-Staff-only search/filter/sort/pagination scope, but make Owner filter choices stable and complete by returning active IT Staff/Administrator owner metadata independent of the current Ticket page, with regression proof that off-page owners remain selectable; keep Staff Ticket Detail operations for Issue #38.
