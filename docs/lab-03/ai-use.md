...
10. Implement Issue #40 as test/regression work only: add the required authentication, Staff/Requester, and Administrator Playwright flows plus REG-01/REG-02, run Playwright against a guarded isolated `_test` database across desktop/tablet/mobile, complete pending UNIT-01 traceability using the production email normalizer, and record only actual local passing/failing evidence without changing product behavior to make tests pass.
