...
10. Implement Issue #37 IT Staff Ticket Queue using TDD and only the approved Queue scope: add the IT-Staff-only `/api/staff/tickets` search/filter/sort/pagination contract with safe invalid-query handling, build the Zen Green desktop table plus tablet/mobile card representation and loading/empty/no-results/forbidden/failure states, preserve role boundaries, and keep Staff Ticket Detail operations for Issue #38.
